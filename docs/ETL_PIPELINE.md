...
The ETL (Extract-Transform-Load) pipeline processes CS2 demo files (`.dem`) into
optimized cache files for real-time input visualization during demo playback.

## Components

- **`src/parsers/etl_pipeline.py`** — Main ETL pipeline implementation. Handles
  extraction, transformation, and loading phases. Includes a CLI for standalone
  execution, auto-detection of players, progress logging, and error handling.
- **`src/parsers/button_decoder.py`** — Decodes CS2 button bitmasks into
  readable key names. Processes subtick timing information. Supports all
  standard CS2 inputs (WASD, mouse, jump, etc.).
- **`src/parsers/cache_manager.py`** — Manages cache file operations
  (save/load). Supports multiple formats: JSON, MessagePack, SQLite. Includes
  cache optimization, validation, and detailed cache statistics.
- **`src/parsers/__init__.py`** — Package initialization with clean exports;
  integrates with the existing `mock_data_generator`.
...
Automatically detects the most active player in a demo. No need to manually
specify a SteamID for single-player POV demos.

### Multiple Cache Formats

| Format      | File Size | Load Speed | Human Readable | Use Case                          |
|-------------|-----------|------------|----------------|-----------------------------------|
| JSON        | 100%      | 1x         | yes            | Development, debugging            |
| MessagePack | 30-50%    | 5-10x      | no             | Production, large files           |
| SQLite      | 40-60%    | 3-5x       | no             | Very large demos, random access   |
...
- Graceful fallback when `demoparser2` is not installed.
...
The ETL pipeline includes graceful fallbacks for development without
`demoparser2`:
...
See [`MOCK_DATA.md`](MOCK_DATA.md) for details on the mock data generator.
